# This file has been created based on "train_simple.py"
...
def read_mass_sequence(seq_len, user):
  seq = []
  with open(f"data/{user}.mass") as f:
    for line in f:
      data = f.read().split('\n')[1:-2]
      maxstart = len(data)-seq_len
      seqstart = random.randint(0,maxstart)
      for line in range(seqstart, seqstart+seq_len):
...
def mass_dataloader(seq_len, batch_size, num_samples):
    data =[]
    for s in range(0, num_samples):
      user = random.randint(0,341)
      s = read_mass_sequence(seq_len, user)
      data.append(s)
...
def main(args):
    ''' Training sequence
    '''
    trn_dataloader = mass_dataloader(MAX_SEQ_LEN, BATCH_SIZE, NUM_MASS_TRN)
    val_dataloader = mass_dataloader(MAX_SEQ_LEN, BATCH_SIZE, NUM_MASS_VAL)
    ...
    model = {
        'g': Generator(num_feats=4, use_cuda=train_on_gpu),
        'd': Discriminator(num_feats=4, use_cuda=train_on_gpu)
    }
